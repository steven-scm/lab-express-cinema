...
// i also the Movie model to insert some movies
const MovieModel = require("./../models/Movie.model");

// should create a bunch of movies


(async function createMovies() {
    try {
        const deletedCount = await MovieModel.deleteMany(); // reset the collection
        console.log(`success : ${deletedCount} movie(s) deleted from database !`);
        const res = await MovieModel.insertMany(movies);
        console.log(`success : ${res.length} movie(s) inserted in database !`);
    } catch (err) {
        console.log("ERROR !");
        console.error(err);
    }
